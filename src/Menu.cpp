...
void Menu::selectGameMode()
{
    if (game == nullptr || game->gameState == GameState::Menu)
    {
        ...
        DrawCircle(config.screenWidth / 2 - config.tileSize / 2 - config.tileSize,
                   config.tileSize + config.tileSize * 2, config.tileSize, headColor);
        DrawRectangle(config.screenWidth / 2 - config.tileSize / 2, fontSize, tempSize, tempSize, GREEN);
        DrawCircle(config.screenWidth / 2 - config.tileSize / 2 + config.tileSize * 3,
                   config.tileSize + config.tileSize * 2, config.tileSize, tailColor);

        // select head color
        const auto isClickedHeadRed = ClickableRectangle({0, fontSize, tempSize, tempSize}, RED).draw().isClicked(
            GetMousePosition());
        if (isClickedHeadRed) headColor = RED;
        const auto isClickedHeadGreen = ClickableRectangle({tempSize, fontSize, tempSize, tempSize}, GREEN).draw().
            isClicked(GetMousePosition());
        if (isClickedHeadGreen) headColor = GREEN;
        const auto isClickedHeadBlue = ClickableRectangle({tempSize * 2, fontSize, tempSize, tempSize}, BLUE).draw().
            isClicked(GetMousePosition());
        if (isClickedHeadBlue) headColor = BLUE;
        const auto isClickedHeadYellow = ClickableRectangle({tempSize * 3, fontSize, tempSize, tempSize}, YELLOW).draw()
            .isClicked(GetMousePosition());
        if (isClickedHeadYellow) headColor = YELLOW;
        const auto isClickedHeadPurple = ClickableRectangle({tempSize * 4, fontSize, tempSize, tempSize}, PURPLE).draw()
            .isClicked(GetMousePosition());
        if (isClickedHeadPurple) headColor = PURPLE;

        // select tail color
        const auto isClickedTailRed = ClickableRectangle({
                                                             config.screenWidth - tempSize * 5, fontSize, tempSize,
                                                             tempSize
                                                         }, RED).draw().isClicked(GetMousePosition());
        if (isClickedTailRed) tailColor = RED;
        const auto isClickedTailGreen = ClickableRectangle({
                                                               config.screenWidth - tempSize * 4, fontSize, tempSize,
                                                               tempSize
                                                           }, GREEN).draw().isClicked(GetMousePosition());
        if (isClickedTailGreen) tailColor = GREEN;
        const auto isClickedTailBlue = ClickableRectangle({
                                                              config.screenWidth - tempSize * 3, fontSize, tempSize,
                                                              tempSize
                                                          }, BLUE).draw().isClicked(GetMousePosition());
        if (isClickedTailBlue) tailColor = BLUE;
        const auto isClickedTailYellow = ClickableRectangle({
                                                                config.screenWidth - tempSize * 2, fontSize, tempSize,
                                                                tempSize
                                                            }, YELLOW).draw().isClicked(GetMousePosition());
        if (isClickedTailYellow) tailColor = YELLOW;
        const auto isClickedTailPurple = ClickableRectangle({
                                                                config.screenWidth - tempSize, fontSize, tempSize,
                                                                tempSize
                                                            }, PURPLE).draw().isClicked(GetMousePosition());
        if (isClickedTailPurple) tailColor = PURPLE;


        if (singleGameButton.isClicked())
        {
            const auto snake = new PlayerSnake(headColor, tailColor, config,
        ...
        {
            const auto snake = new PlayerSnake(headColor,BLUE, config,
                                               Vector2{0, static_cast<float>(config.gridHeight) / 2});
