...
void LANGame::draw()
{
    if (gameState == GameState::Start)
    {
        ...
        if (startButton.isClicked(GetMousePosition()))
        {
            std::thread([this]()
            {
                if (isHost)
        ...
        if (hostButton.isClicked(GetMousePosition()))
        {
            if (!isContextRun)
            {
                isHost = true;
        ...
        else if (guestButton.isClicked(GetMousePosition()))
        {
            if (!isContextRun)
            {
                isHost = false;
    ...
    else if (gameState == GameState::Playing)
    {
        ...
        if (pauseButton.isClicked(GetMousePosition()))
        {
            snake->direction = {0, 0};
        }
    }
    else if (gameState == GameState::Paused)
    {
        ...
        if (menuButton.isClicked(GetMousePosition()))
        {
            rapidjson::Document document(rapidjson::kObjectType);
            document.AddMember("dataType", rapidjson::StringRef("state"), document.GetAllocator());
